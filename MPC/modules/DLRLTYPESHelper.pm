package DLRLTYPESHelper;

# ************************************************************
# Description   : Command helper for OSPL ospldcg generated files. Required
#                 because if you thought the idlpp output file naming conventions
#                 were weird, you hadn't actually seen anything yet.
#
...
#                 DLRLTypes_Files entries in .mpc files.
# $Id:$
# ************************************************************
...
sub get_output {
  ...
  ## syntax.
  my ($self, $types_idl_file, $ospldcg_args)  = @_;
  ...
  my $left_over_args = '';
  my $ret = '';
  ($ret, $left_over_args) = _GetOptionsFromString($ospldcg_args,
                                                'o=s' => \$output_dir,
                                                'l=s' => \$language,
                                                'dcps=s' => \$dcps_types_file,
                                                'gendcps' => \$gen_dcps_types);
  ...
  if (uc($language) eq 'SACPP')
  {
    push @filenames, "$output_dir/ccpp_$base" . ".h";
    push @filenames, "$output_dir/ccpp_$base" . "_abstract.cpp";
    push @filenames, "$output_dir/ccpp_$base" . "_abstract.h";
    push @filenames, "$output_dir/ccpp_$base" . "_impl.cpp";
    push @filenames, "$output_dir/ccpp_$base" . "_impl.h";
    push @filenames, "$output_dir/ccpp_$base" . "Dlrl_impl.cpp";
    push @filenames, "$output_dir/ccpp_$base" . "Dlrl_impl.h";
    push @filenames, "$output_dir/$base" . ".cpp";
    push @filenames, "$output_dir/$base" . ".h";
    push @filenames, "$output_dir/$base" . "Dlrl.cpp";
    push @filenames, "$output_dir/$base" . "Dlrl.h";

    if ($gen_dcps_types == 1)
    {
      require "TYPESUPPORTHelper.pm";
      $dcps_types_helper = TYPESUPPORTHelper->new() unless defined $dcps_types_helper;
      push @filenames, @{$dcps_types_helper->get_output($dcps_types_file, "-l cpp -S -d $output_dir")};
    }
  }
  return \@filenames;
}
