...
<xsl:if test="$isNonIncludedSharedValueDef='true'">
    ...
    <xsl:variable name="filename">
        <xsl:call-template name="string-search-replace">
            <xsl:with-param name="text"><xsl:value-of select="@NAME"/><xsl:text>StrMap</xsl:text></xsl:with-param>
 ...
 */
public final class <xsl:value-of select="$className"/>StrMap extends DDS.StrMap{

    //disallow instantiation of this class
    private <xsl:value-of select="$className"/>StrMap(){}
     ...
     */
    public <xsl:value-of select="$classNameFullyQualified"/> get(java.lang.String key) throws DDS.AlreadyDeleted, DDS.NoSuchElement{
     ...
     * @throws DDS.AlreadyDeleted if the owner of the Map has already been deleted.
     */
    public void put(java.lang.String key, <xsl:value-of select="$classNameFullyQualified"/> value) throws DDS.AlreadyDeleted, DDS.PreconditionNotMet{
        jniPut(key, value);
    }
}
