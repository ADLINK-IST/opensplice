...
for each in `cat $LIST`; do
   ...
   echo === $each;
   echo ============================================
   git add $DCPS/$each/$LANG/$CORBA_DIR/RUN.bat
   git add $DCPS/$each/$LANG/$SA_DIR/RUN.bat
done
git add $DCPS/Durability/$LANG/$CORBA_DIR/start.bat
git add $DCPS/Durability/$LANG/$SA_DIR/start.bat
