...
echo Building sacpp_builtintopics_sub.%projsfx%
if %use_devenv% == 1 (
   devenv "%OSPL_HOME%examples\All_Standalone_C_and_CPlusPlus.sln" /%1 Release /project "%OSPL_HOME%examples\dcps\BuiltInTopics\cpp\standalone\sacpp_builtintopics_sub.%projsfx%"
) else (
   msbuild sacpp_builtintopics_sub.%projsfx% /p:Configuration=Release
)
