...
echo Building ISO_Cxx_V2_BuiltinTopics_Impl.%projsfx%
if %use_devenv% == 1 (
   devenv "%OSPL_HOME%examples\DCPS_ISO_CPlusPlus.sln" /%1 Release /project "%OSPL_HOME%examples\dcps\BuiltInTopics\isocpp2\ISO_Cxx_V2_BuiltinTopics_Impl.%projsfx%"
) else (
   msbuild ISO_Cxx_V2_BuiltinTopics_Impl.%projsfx% /p:Configuration=Release
)
...
echo Building ISO_Cxx_V2_BuiltinTopics_Sub.%projsfx%
if %use_devenv% == 1 (
   devenv "%OSPL_HOME%examples\DCPS_ISO_CPlusPlus.sln" /%1 Release /project "%OSPL_HOME%examples\dcps\BuiltInTopics\isocpp2\ISO_Cxx_V2_BuiltinTopics_Sub.%projsfx%"
) else (
   msbuild ISO_Cxx_V2_BuiltinTopics_Sub.%projsfx% /p:Configuration=Release
)
