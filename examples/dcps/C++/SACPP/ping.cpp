...
typedef DDS::Boolean pong_handler (unsigned int max_count);
    ...
    )
{
    ...
    DDS::Boolean                         finish_flag = false;
    DDS::Boolean                         timeout_flag = false;
    DDS::Boolean                         terminate = false;
    ...
    case 't': 
        ...
        // Create datawriter
        dw = p->create_datawriter(PP_quit_topic, DATAWRITER_QOS_DEFAULT, NULL, DDS::ANY_STATUS);
    ...
    for (block = 0; block < nof_blocks ; block++) {
	    ...
	    if (!terminate) {
                ...
                while (!(timeout_flag || finish_flag)) {
                    w.wait (conditionList.inout(), wait_timeout);
