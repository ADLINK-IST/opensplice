			...
			//private pingpong.PP_min_msg PP_min_dataList = new pingpong.PP_min_msg();
		    private pingpong.PP_min_msg[] PP_min_dataList = null;
		
		//	private pingpong.PP_seq_msg PP_seq_dataList = new pingpong.PP_seq_msg();
		private pingpong.PP_seq_msg[] PP_seq_dataList = null;

//		    private pingpong.PP_string_msg PP_string_dataList = new pingpong.PP_string_msg();
			private pingpong.PP_string_msg[] PP_string_dataList = null;
		
//		    private pingpong.PP_fixed_msg PP_fixed_dataList = new pingpong.PP_fixed_msg();
 private pingpong.PP_fixed_msg[] PP_fixed_dataList = null;
		
//		    private pingpong.PP_array_msg PP_array_dataList = new pingpong.PP_array_msg();
private pingpong.PP_array_msg[] PP_array_dataList = null;
		
//		    private pingpong.PP_quit_msg PP_quit_dataList = new pingpong.PP_quit_msg();
private pingpong.PP_quit_msg[] PP_quit_dataList = null;
			...
			private time postTakeTime = new time();
				...
				public void run(String[] args) {
					...
					DDS.ICondition[] conditionList = null; //new DDS.ICondition();
					DDS.IWaitSet w;
					...
					/*if (args.Length != 0) {
						if (args.Length != 5) {
							...
							System.Console.WriteLine("Usage: java ping [blocks blocksize topic_id WRITE_PARTITION READ_PARTITION]");
							return;
						}
                    ...
                    dp = dpf.CreateParticipant(myDomain, ref dpQos);//, null, DDS.StatusKind.Any);// as DDS.OpenSplice.DomainParticipant;
                   ...
                   /*
					...
					PP_min_topic = dp.CreateTopic("PP_min_topic", "pingpong::PP_min_msg",
							ref tQos);//, null, DDS.StatusKind.Any);
                    /* Create datawriter */
					PP_min_writer = p.CreateDataWriter(PP_min_topic, ref dwQos)//, null,DDS.StatusKind.Any)
									as pingpong.PP_min_msgDataWriter;
					...
					PP_seq_topic = dp.CreateTopic("PP_seq_topic", "pingpong::PP_seq_msg",ref tQos);
							//tQos.value, null, DDS.StatusKind.Any);
                   /* Create datawriter */
					PP_seq_writer = p
							.CreateDataWriter(PP_seq_topic, ref dwQos)as pingpong.PP_seq_msgDataWriter;
					...
					PP_string_topic = dp.CreateTopic("PP_string_topic",
							"pingpong::PP_string_msg", ref tQos);//, null,
							//DDS.StatusKind.Any);
                    /* Create datawriter */
					...
					//PP_fixed_topic = dp.CreateTopic("PP_fixed_topic",
						//	"pingpong::PP_fixed_msg", tQos.value, null,
							//DDS.StatusKind.Any);
					...
					PP_array_topic = dp.CreateTopic("PP_array_topic",
							"pingpong::PP_array_msg", ref tQos);
		...
		//			assert (result == RETCODE_OK.value);
					...
					PP_quit_topic = dp
							.CreateTopic("PP_quit_topic", "pingpong::PP_quit_msg", ref tQos);
									//tQos.value, null, DDS.StatusKind.Any);
					...
					for (block = 0; block < nof_blocks; block++) {
						...
						if (!terminate) {
							...
							if (block == 0) {
								...
								//System.Console.WriteLine(DateFormat.getDateTimeInstance().format(
									//	new java.util.Date()));
		...
		//			drQos = null;
				...
				if (amount != 0) {
					...
					if (PP_min_dataList[0].count < nof_cycles) {
						preWriteTime.timeGet();
						dds_result = PP_min_writer.Write(PP_min_dataList[0],
		...
		//				assert (dds_result == RETCODE_OK.value);
					...
					roundtrip.add_stats(postTakeTime.sub(roundTripTime));
					roundTripTime.set(preWriteTime.get());
					dds_result = PP_array_reader.ReturnLoan(ref PP_array_dataList,
							ref infoList);
		//			assert (dds_result == RETCODE_OK.value);
				} else {
					System.Console
							.WriteLine("PING: PING_array triggered, but no data available");
				}
				return result;
			}
	...
	//				assert (dds_result == RETCODE_OK.value);
					...
					System.Console
							.WriteLine("PING: PING_seq triggered, but no data available");
				}
	...
	//			assert (dds_result == RETCODE_OK.value);
				...
				System.Console
						.WriteLine("PING: PING_string triggered, but no data available");
			}
			return result;
		}
				}
