...
{
   char* typeName = (char*) HelloWorldData_MsgTypeSupport_get_type_name(typeSupport);

   g_status = HelloWorldData_MsgTypeSupport_register_type(typeSupport, g_domainParticipant, typeName);
...
void writeMessageSample(DDS_DataWriter dataWriter,
   ...
   HelloWorldData_Msg* messageSample)
{
   g_status = HelloWorldData_MsgDataWriter_write(dataWriter, messageSample, userHandle);
