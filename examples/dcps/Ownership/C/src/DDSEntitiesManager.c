...
{
   char* typeName = (char*) OwnershipData_StockTypeSupport_get_type_name(typeSupport);
...
void setOwnershipStrength(DDS_DataWriter dataWriter, DDS_long strength)
{
  ...
  memset(&dw_qos, 0, sizeof(DDS_DataWriterQos));
  g_status = DDS_DataWriter_get_qos(dataWriter, &dw_qos);
  checkStatus(g_status, "DDS_DataWriter_get_qos");
  dw_qos.ownership_strength.value = strength;
  g_status = DDS_DataWriter_set_qos(dataWriter, &dw_qos);
  checkStatus(g_status, "setQos");
}
