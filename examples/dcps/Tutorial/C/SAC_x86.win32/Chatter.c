    ...
    char *argv[]) 
{
    ...
    if (chatterName) {
        ...
        sprintf_s(ns.name, Chat_MAX_NAME+1, "Chatter %d", ownID);
    }
    ...
    if (ownID == TERMINATION_MESSAGE) {
        sprintf_s (msg->content, MAX_MSG_LEN, "Termination message.");
    } else { 
        sprintf_s (msg->content, MAX_MSG_LEN, "Hi there, I will send you %d more messages.", NUM_MSG);
    }
    ...
    for (i = 1; i <= NUM_MSG && ownID != TERMINATION_MESSAGE; i++) {
        msg->index = i;
        sprintf_s ( msg->content, MAX_MSG_LEN, "Message no. %d", msg->index);
        printf("Writing message: %s\n", msg->content);
        status = Chat_ChatMessageDataWriter_write(talker, msg, userHandle);
        checkStatus(status, "Chat_ChatMessageDataWriter_write");
        Sleep (1000); /* do not run so fast! */
