...
if countTest.count_test == False:
    from SequenceOfStruct.basic.module_SequenceOfStruct import SequenceOfStruct_struct, Inner

class Test(unittest.TestCase):


    def testCopyInCopyOut(self):
        data = SequenceOfStruct_struct(
            long1 = 13,
            seq1 = [Inner(short1=11,double1=1.1),
                    Inner(short1=12,double1=1.2),
                    Inner(short1=13,double1=1.3)])
        print('data: ' + str(data))
        ...
        for i in range(len(data.seq1)):
            self.assertEqual(data.seq1[i].short1, data1.seq1[i].short1, 'seq1[%d].short1'%(i))
            self.assertEqual(data.seq1[i].double1, data1.seq1[i].double1, 'seq1[%d].double1'%(i))
