...
c_array new_v_message_qos (const nn_xqos_t *xqos, c_collectionType qostype)
{
  ...
  assert (xqos->present & QP_WRITER_DATA_LIFECYCLE);
  {
  ...
  msgqos = c_newArray (qostype, dst - qosbase);
  if (msgqos != NULL)
...
int fill_v_message_qos (const struct proxy_writer *pwr, v_message vmsg, const struct nn_prismtech_writer_info *wri, os_int64 tstamp, os_int64 seq, unsigned statusinfo, int have_data)
{
  ...
  if (statusinfo == 0)
    assert (have_data);
  else if (statusinfo == NN_STATUSINFO_UNREGISTER)
    v_stateSet (v_nodeState (vmsg), L_UNREGISTER);
  else if (statusinfo == NN_STATUSINFO_DISPOSE)
    v_stateSet (v_nodeState (vmsg), L_DISPOSED);
  else
  {
    nn_log (LC_FATAL, "received unknown statusinfo: %d\n", statusinfo);
    abort ();
  }
  return 1;
}
