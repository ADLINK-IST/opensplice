...
static topic_t deftopic1 (const char *name, C_STRUCT(v_topic) const * const ospl_topic, const char *typename, C_STRUCT(c_type) const * const type, int nkeys, char const * const *keys)
{
  ...
  if (tp != NULL)
  {
    nn_log (LC_TRACE, "deftopic1: reusing definition for %s\n", name_typename);
    os_free (name_typename);
    return tp;
  }

  /* Define new one */
  nn_log (LC_TRACE, "deftopic1: new topic %s\n", name_typename);
...
static topic_t deftopic4u (c_base base)
{
  ...
  return deftopic1 ("....4u....", NULL, "nn_osplserModule::type4u", c_resolve (base, "nn_osplserModule::type4u"), 4, keys);
}
...
static topic_t deftopicpmd (c_base base)
{
  ...
  osplser_topicpmd_type = c_resolve (base, "nn_osplserModule::pmd");
  osplser_topicpmd_value_type = c_specifier (c_metaResolve ((c_metaObject) osplser_topicpmd_type, "value"))->type;
  return deftopic1 ("....pmd....", NULL, "nn_osplserModule::pmd", osplser_topicpmd_type, 4, keys);
}

int osplser_init (void *vbase)
{
  c_base base = vbase;
  if (!loadnn_osplserModule (base))
    return -1;
...
serdata_t serialize_raw (serstatepool_t pool, const struct topic *tp, const void *data, unsigned statusinfo, os_int64 timestamp, const struct nn_prismtech_writer_info *wri)
{
  serdata_t d = serialize_raw_private (pool, tp, data);
  if (d)
    serstate_set_msginfo (d->u.st, statusinfo, timestamp, wri);
  return d;
}
...
static unsigned statusinfo_from_msg (C_STRUCT (v_message) const *msg)
{
  switch (v_nodeState ((v_message) msg))
  {
    ...
    default:
      nn_log (LC_WARNING, "statusinfo_from_msg: unhandled message state: %u\n", (unsigned) v_nodeState ((v_message) msg));
      return 0;
  }
}
...
void serstatepool_free (serstatepool_t pool)
{
  ...
  nn_log (LC_TRACE, "serstatepool_free(%p) nalloced %d nfree %d\n", pool, pool->nalloced, pool->nfree);
  os_free (pool);
}
...
#ifndef NDEBUG
  st->data->v.msginfo.have_wrinfo = -1;
#endif
...
void serstate_set_msginfo (serstate_t st, unsigned statusinfo, os_int64 timestamp, const struct nn_prismtech_writer_info *wri)
{
  serdata_t d = st->data;
    ...
    case C_SEQUENCE:
      {
        ...
        else
        {
          ...
          for (i = 0; i < length; i++)
          {
            int sersize1 = serialize1 (st, subtype, src1, off);
            if (sersize1 < 0)
              return sersize1;
            sersize += sersize1;
            src1 += datasize1;
