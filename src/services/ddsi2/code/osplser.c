...
#include "avl.h"
#include "osplser.h"
#include "osplserModule.h"

/* these just for rtps_trace() */
#include "rtps.h"
#include "protocol.h"
...
static serstate_t serstate_new (serstatepool_t pool, const struct topic *topic);
static serstate_t serstate_grab (serstatepool_t pool, const struct topic *topic);
static void serstate_release (serstate_t st);
static void serstate_free (serstate_t st);
static unsigned serialize1 (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off);
...
static unsigned short toBE2u (unsigned short x) { return bswap2u (x); }
static unsigned short fromBE2u (unsigned short x) { return bswap2u (x); }
static unsigned toBE4u (unsigned x) { return bswap4u (x); }
static unsigned fromBE4u (unsigned x) { return bswap4u (x); }
static unsigned long long toBE8u (unsigned long long x) { return bswap8u (x); }
static unsigned long long fromBE8u (unsigned long long x) { return bswap8u (x); }
#else
static unsigned short toBE2u (unsigned short x) { return x; }
static unsigned short fromBE2u (unsigned short x) { return x; }
static unsigned toBE4u (unsigned x) { return x; }
static unsigned fromBE4u (unsigned x) { return x; }
static unsigned long long toBE8u (unsigned long long x) { return x; }
static unsigned long long fromBE8u (unsigned long long x) { return x; }
#endif
...
static unsigned ceiling_power_of_2 (unsigned x)
{
  if (ispowerof2 (x))
    return x;
  else
...
static topic_t deftopic1 (const char *name, C_STRUCT(v_topic) const * const ospl_topic, const char *typename, C_STRUCT(c_type) const * const type, int nkeys, char const * const *keys)
{
  topic_t tp;
  int i;
  ...
  if (tp != NULL)
  {
    rtps_trace (DBGFLAG_TRACING, "deftopic1: reusing definition for %s\n", name_typename);
    os_free (name_typename);
    return tp;
  }

  /* Define new one */
  rtps_trace (DBGFLAG_TRACING, "deftopic1: new topic %s\n", name_typename);
  if ((maxstrlengths = os_malloc (nkeys * sizeof (*maxstrlengths))) == NULL)
    goto fail00;
  if ((tp = os_malloc (offsetof (struct topic, keys) + (nkeys+1) * sizeof (tp->keys[0]))) == NULL)
    goto fail0;
  avl_init_node (tp, parent);
  ...
  {
    unsigned short *tmp;
    if ((tmp = os_malloc (nkeys * sizeof (*tmp))) == NULL)
      goto fail4;
    for (i = 0; i < nkeys; i++)
...
#define MAX_NKEYS 32 /* arbitrary limit out of laziness */
  ...
  ospl_typename = c_metaObject (ospl_type)->name;
  ospl_topicname = v_entity ((v_topic) ospl_topic)->name;
  tp = deftopic1 (ospl_topicname, ospl_topic, ospl_typename, ospl_type, nkeys, (char const * const *) keys);
  os_free (keystr_copy);
  return tp;
 fail:
  os_free (keystr_copy);
...
static void freetopic_helper (topic_t tp)
{
  c_free (tp->type);
  if (tp->ospl_topic)
    c_free (tp->ospl_topic);
...
{
  /* Special case for generating serdata in PMD and SEDP messages,
     which itself is only required because the rtps.c expects serdata
     to be present in Data messages. To be removed eventually. A topic
     consisting of a 4 unsigned 32-bit integers, all part of the
     key. This corresponds exactly to the key of SEDP and PMD data
     ... */
  char const *keys[] = { "a", "b", "c", "d" };
  return deftopic1 ("....4u....", NULL, "osplserModule::type4u", c_resolve (base, "osplserModule::type4u"), 4, keys);
}

int osplser_init (void *vbase)
{
  c_base base = vbase;
  if (!loadosplserModule (base))
    return -1;
  avl_init_indkey (&topictree, offsetof (struct topic, avlnode), offsetof (struct topic, name_typename), (int (*) (const void *, const void *)) strcmp, 0, (void (*) (void *)) freetopic_helper);
  osplser_topic4u = deftopic4u (base);
  messageQosType = c_metaArrayTypeNew(c_metaObject(base), "C_ARRAY<c_octet>", c_octet_t(base), 0);
  return 0;
}
...
static void gen_keyhash_md5 (struct serdata const * const d, char keyhash[16])
{
  ...
  for (i = 0; i < tp->nkeys; i++)
  {
      ...
      case TC_STRINGINLINE:
	{
	  ...
	  unsigned len;
	  assert (!(d->isstringref & (1u << ki->keyseroff)));
	  str = (const struct cdrstring *) src;
#if KEYHASH_HAS_PADDING
...
#endif
#if PLATFORM_IS_LITTLE_ENDIAN
	  md5_append (&md5st, (const unsigned char *) &len, sizeof (len));
...
void serdata_keyhash (const struct serdata *d, char keyhash[16])
{
  ...
  if (d->isstringref == 0 && d->u.st->topic->keysersize <= 16)
...
int serdata_cmp (const struct serdata *a, const struct serdata *b)
{
  ...
  assert (a->u.st->topic->keysersize == b->u.st->topic->keysersize);
  while (x)
  {
    unsigned q = myffs (x) - 1;
    x ^= x & -x; /*/ x & -x: lowest set bit => xor clears bit q */
    assert (q >= p);
    assert (p < a->u.st->topic->keysersize);
    assert (q < a->u.st->topic->keysersize);
    if (q > p)
    {
  ...
  if (p < a->u.st->topic->keysersize)
    return memcmp (a->key + p, b->key + p, a->u.st->topic->keysersize - p);
  else
    return 0;
}
...
int serdata_size (const struct serdata *serdata)
{
  ...
  return (int) (st->pos - offsetof (struct serdata, hdr));
}

int serdata_keyhash_exact_p (const struct serdata *serdata)
{
  return (serdata->u.st->topic->keysersize <= 16 && serdata->isstringref == 0);
}
...
{
  serstate_t st = serstate_grab (pool, tp);
...
serdata_t serialize (serstatepool_t pool, const struct topic *tp, C_STRUCT (v_message) const *msg)
{
  ...
  return serialize_raw (pool, tp, udata);
}
...
void serstatepool_free (serstatepool_t pool)
{
  ...
  rtps_trace (DBGFLAG_TRACING, "serstatepool_free(%p) nalloced %d nfree %d\n", pool, pool->nalloced, pool->nfree);
  os_free (pool);
}
...
{
  st->pos = offsetof (struct serdata, data);
  st->keyidx = 0;
  st->topic = topic;
...
static serstate_t serstate_new (serstatepool_t pool, const struct topic *topic)
{
  ...
  pool->nalloced++;
  st->size = alignup (offsetof (struct serdata, data) + 128, 128);
  st->data = os_malloc (st->size);
  st->pool = pool;
  st->data->u.st = st;
...
static void *serstate_append (serstate_t st, unsigned n)
{
  ...
  if (st->pos + n > st->size)
  {
    ...
    serdata_t data1 = os_realloc (st->data, size1);
    if (data1 == NULL)
    {
...
static void *serstate_append_specific_alignment (serstate_t st, unsigned n, unsigned a)
{
  ...
  if (p && st->pos > pos0)
    memset ((char *) st->data + pos0, 0, st->pos - pos0);
  return p;
}
...
static void copykey (serstate_t st, const char *p, unsigned sz)
{
...
static unsigned serprim (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off)
{
  char *p;
  ...
  if (off == st->topic->keys[st->keyidx].off)
    copykey (st, p, type->size);
  return (unsigned) type->size;
}

static unsigned serenum (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off)
{
  char *p;
...
static unsigned sercoll (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off)
{
  ...
  switch (ctype->kind)
  {
	...
	*((unsigned *) p) = n;
	...
	else
	{
	  ...
	  for (i = 0; i < length; i++)
	  {
	    unsigned sersize1 = serialize1 (st, subtype, src1, off);
	    if (sersize1 == 0)
	      return 0;
	    sersize += sersize1;
	    src1 += datasize1;
...
static unsigned serstruct (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    unsigned sersize1 = serialize1 (st, subtype, data + disp, off + disp);
    if (sersize1 == 0)
      return 0;
    sersize += sersize1;
  }
...
static unsigned serunion (serstate_t st, C_STRUCT(c_type) const * const type, const char *data, unsigned off)
{
...
static unsigned serialize1 (serstate_t st, C_STRUCT(c_type) const * const type_x, const char *data, unsigned off)
{
...
static int deserstringG_unaligned (C_STRUCT(c_collectionType) const * const ctype, char *dst, const char *src, unsigned srcoff, unsigned srcsize, int swap)
{
  ...
  else if (n == 0)
  {
    ...
    *((c_string *) dst) = NULL;
    ...
    return -1;
  else if (ctype->maxSize > 0 && n > (unsigned) ctype->maxSize)
    return -1;
...
static int deserstringP_unaligned (C_STRUCT(c_collectionType) const * const ctype, char **dst, int *dstsize, const char *src, unsigned srcoff, unsigned srcsize, int swap)
{
  ...
  {
    mysnprintf (dst, dstsize, "(string length %u out of bounds wrt serdata)", n);
    return -1;
  }
  else if (ctype->maxSize > 0 && n > (unsigned) ctype->maxSize)
  {
    mysnprintf (dst, dstsize, "(string length %u out of bounds wrt type)");
    return -1;
  }
  else if (src[srcoff + n-1] != '\0')
  {
...
static int desercollG (C_STRUCT(c_type) const * const type, char *dst, const char *src, unsigned srcoff, unsigned srcsize, int swap)
{
  ...
  switch (ctype->kind)
  {
	...
	else /* C_SEQUENCE or C_ARRAY with ctype->maxSize == 0 (cf sercoll()) */
	{
	  ...
	  if ((c_long) n < 0 || n > (srcsize - srcoff) / size1)
	    return -1;
	}
	if (ctype->kind == C_ARRAY && ctype->maxSize > 0)
	{
	...
	else
	{
	  ...
	  /* For bad compilers and processors with static branch
	     ...
	     duplicated, one the swapped & one for the non-swapped
	     case */
	  for (i = 0; i < n; i++)
	  {
	    int off1;
...
{
  if (!serdata->u.st->justkey)
    return prettyprint_raw (dst, dstsize, serdata->u.st->topic, &serdata->hdr, serdata->u.st->pos - offsetof (struct serdata, hdr));
  else
    return prettyprint_key (dst, dstsize, serdata->u.st->topic, &serdata->hdr, serdata->u.st->pos - offsetof (struct serdata, hdr));
}
...
int serdata_verify (serdata_t serdata, C_STRUCT (v_message) const *srcmsg)
{
  ...
  const void *a;
  const void *b;
  v_message msg = deserialize (serdata->u.st->topic, &serdata->hdr, serdata->u.st->pos - offsetof (struct serdata, hdr));
  if (msg == NULL)
    goto fail;
