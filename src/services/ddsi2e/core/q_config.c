...
#if ! LITE
  { ATTR ("Resolution"), 1, "1s", RELOFF (config_channel_listelem, resolution), 0, uf_duration_ms_1s, 0, pf_duration,
...
static const struct cfgelem control_topic_cfgattrs[] = {
  { ATTR ("enable"), 1, "false", ABSOFF (enable_control_topic), 0, uf_boolean, 0, pf_boolean,
    "<p>This attribute controls whether the DDSI2E control topic is defined and acted upon by DDSI2</p>" },
  { ATTR ("initialreset"), 1, "inf", ABSOFF (initial_deaf_mute_reset), 0, uf_duration_inf, 0, pf_duration,
    "<p>This attribute sets the time until the deaf and mute settings are automatically reset to false</p>" },
  END_MARKER
};
...
<p>See also General/AllowMulticast.</p>" },
  ...
  { LEAF ("ResponsivenessTimeout"), 1, "inf", ABSOFF (responsiveness_timeout), 0, uf_duration_inf, 0, pf_duration,
    "<p>This element controls for how long an unresponsive reader can block the transmit thread by failing to acknowledge data when a writer's DDSI2E write cache is full. If after this time the writer's cache has not shrunk to below the low-water mark, the reader is considered unresponsive and degraded to unreliable. It will be restored to reliable service once it resumes acknowledging samples.</p>" },
#endif
...
<p>See also Internal/RetransmitMerging.</p>" },
  ...
  { LEAF_W_ATTRS ("HeartbeatInterval", heartbeat_interval_attrs), 1, "100 ms", ABSOFF (const_hb_intv_sched), 0, uf_duration_inf, 0, pf_duration,
    "<p>This element sets the base interval for the asynchronous, periodic writer heartbeats when unacknowledged data is present in its writer history cache. The actual interval is dynamically adjusted, the attributes of this element allow further configuration.</p>" },
  ...
  { LEAF ("ForwardRemoteData"), 1, "default", ABSOFF (forward_remote_data), 0, uf_boolean_default, 0, pf_boolean,
    "<p>This element controls whether DDSI2E forwards data received from other network services in the domain.</p>" },
#endif
#if LITE
...
static const struct cfgelem root_cfgelems[] = {
  ...
  { "DDSI2E", ddsi2_cfgelems, NULL, NODATA, NULL },
...
static const struct unit unittab_bandwidth_bps[] = {
  ...
  { "GiB/s", 8 * (os_int64) 1073741824 }, { "GiBps", 8 * (os_int64) 1073741824 },
  { "GB/s", 8 * (os_int64) 1000000000 }, { "GBps", 8 * (os_int64) 1000000000 },
  { NULL, 0 }
};
...
static int uf_bandwidth (struct cfgst *cfgst, void *parent, struct cfgelem const * const cfgelem, UNUSED_ARG (int first), const char *value)
{
  ...
  if (strncmp (value, "inf", 3) == 0) {
    ...
    *elem = (os_uint32) (bandwidth_bps / 8);
    return 1;
  }
}
#endif
...
{
  const os_uint32 *elem = cfg_address (cfgst, parent, cfgelem);
  if (*elem == 0)
    cfg_log (cfgst, "inf%s", is_default ? " [def]" : "");
  else
...
)
{
  ...
  if (configfile)
  {
    ...
    while ((tok = os_strsep (&cursor, ",")) != NULL)
    {
      ...
      if ((qx = qhxml_newfp (fp, tok, cfgst, proc_elem_open, proc_attr, proc_elem_data, proc_elem_close)) == NULL)
      {
        (void)fclose (fp);
        os_free (copy);
        os_free (cfgst);
  ...
  {
    NN_ERROR0 ("q_config_init: u_participantGetConfiguration failed");
...
void q_config_fini (void)
{
  if (config.valid)
  {
    ...
    {
      (void)fclose (config.tracingOutputFile);
    }
    memset (&config, 0, sizeof (config));
  }
}
