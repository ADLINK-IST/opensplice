...
error_string(UTL_Error::ErrorCode c)
{
   switch (c)
   {
         ...
         /* More intelligible message printed by error routine */
         return "interface not declared in same file as forward declared: ";
         ...
         case UTL_Error::EIDL_PERFECT_HASH:
         return GTDEVEL("method names too similar for perfect hashing in: ");
...
// declared forward but not yet defined
void
UTL_Error::value_inheritance_fwd_error(AST_Value *f)
{
   idl_error_header(EIDL_INHERIT_FWD_ERROR, f->line(), f->file_name());
   cerr << GTDEVEL(" valuetype cannot inherit from forward declared valuetype ");
   f->local_name()->dump(cerr);
   cerr << "\n";
   idl_global->set_err_count(idl_global->err_count() + 1);
}
...
// declared forward but not yet defined
void
UTL_Error::value_inheritance_fwd_error(AST_Interface *f)
{
   idl_error_header(EIDL_INHERIT_FWD_ERROR, f->line(), f->file_name());
   cerr << GTDEVEL(" valuetype cannot inherit from forward declared interface ");
   f->local_name()->dump(cerr);
   cerr << "\n";
   idl_global->set_err_count(idl_global->err_count() + 1);
}
...
// Report an attempt to inherit from something other than an interface
void
UTL_Error::value_inheritance_error(AST_Decl *d)
{
   idl_error_header(EIDL_CANT_INHERIT, d->line(), d->file_name());
   cerr << GTDEVEL(" valuetype attempts to inherit from ");
   d->dump(cerr);
   cerr << "\n";
   idl_global->set_err_count(idl_global->err_count() + 1);
}
...
void UTL_Error::illegal_identifier (const char *id)
{
   idl_error_header
   (
      EIDL_ILLEGAL_ID,
...
void
UTL_Error::warning_msg(const char *s)
{
   if (idl_global->warn())
   {
      idl_error_header(EIDL_SYNTAX_ERROR, idl_global->lineno(), idl_global->filename());
      cerr << s << endl;
   }
}

void
UTL_Error::error_msg(const char *s)
{
   idl_error_header(EIDL_SYNTAX_ERROR, idl_global->lineno(), idl_global->filename());
   cerr << s << endl;
   idl_global->set_err_count(idl_global->err_count() + 1);
