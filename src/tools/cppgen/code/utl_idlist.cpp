...
UTL_IdList::compare(UTL_IdList* that) const
{
   ...
   while (!result)
   {
      ...
      {
         // let's compare this component
         Identifier * thisid = ithis.item();
         Identifier * thatid = ithat.item();

         // this identifier isn't the same

         if (!thisid->compare(thatid))
         {
            break;
         }
      }
