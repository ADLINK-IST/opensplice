...
public:
   ...
   virtual AST_Opaque *add_opaque(AST_Opaque *o);
   virtual AST_Value *add_valuetype(AST_Value *v);
   virtual AST_ValueFwd *add_valuetype_fwd(AST_ValueFwd *f);
   virtual AST_StateMember *add_state_member(AST_StateMember *m);
   virtual AST_Initializer *add_initializer(AST_Initializer *i);
   virtual AST_BoxedValue *add_boxed_valuetype(AST_BoxedValue *b);
   ...
   UTL_Pragmas &get_pragmas();
   void set_pragmas(const UTL_Pragmas &p);
...
private:
   ...
   // Scope management
   static UTL_Scope *lookup_combined_scope(const UTL_ScopedName&);
   UTL_ScopedName *pd_scoped_name; // What's my name? (just reference)
   static UTL_Scope *pd_scopes_first; // Store all scopes JOEY remove this
   static UTL_Scope *pd_scopes_last; // Store all scopes JOEY remove this
   UTL_Scope *pd_scopes_next;
   UTL_Scope *pd_combined_scope; // the scope containing combined decls
   AST_Decl_Map *pd_combined_decls; // all decls for all scopes of this name
   ...
   // Storage for pragma info affecting this scope:
...
private:
   ...
   virtual AST_ValueFwd *fe_add_valuetype_fwd(AST_ValueFwd *f);
...
public:
   ...
   virtual ~UTL_ScopeActiveIterator()
   {}
...
//  iterator for a UTL_Scope node

class UTL_ScopeListIterator
{

public:

   // Constructor(s)
   UTL_ScopeListIterator();

   virtual ~UTL_ScopeListIterator()
   {}
...
private:
   ...
   // Data
   UTL_Scope *m_current; // Scope to iterate over
};
