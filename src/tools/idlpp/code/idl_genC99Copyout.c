    ...
    void *userData)
{
    ...
    c_char *scopedNameC = idl_scopeStackC (scope, "_", name);
    ...
    os_boolean stacRequested)
{
    ...
    case idl_octet:
        {
            c_char *cTypeName = idl_corbaCTypeFromTypeSpec (idl_typeSpec(typeBasic));
            idl_fileOutPrintf (idl_fileCur(), "    %s%s = (%s)%s%s;\n",
                    to_id, cIdName, cTypeName, from_id, cIdName);
            os_free(cTypeName);
        }
        break;
    case idl_string:
        ...
        if (maxlen == 0) {
            idl_fileOutPrintf (idl_fileCur(), "    DDS_string_replace (%s%s ? %s%s : \"\", &%s%s);\n",
                    from_id, cIdName, from_id, cIdName, to_id, cIdName);
        } else {
            idl_fileOutPrintf (idl_fileCur(), "    if (%s%s) {\n",
                    from_id, cIdName);
            idl_fileOutPrintf (idl_fileCur(), "        strncpy (&%s%s[0], %s%s, %d);\n",
                    to_id, cIdName, from_id, cIdName, maxlen+1);
            idl_fileOutPrintf (idl_fileCur(), "        %s%s[%d]= '\\0';\n",
                    to_id, cIdName, maxlen);
            idl_fileOutPrintf (idl_fileCur(), "    } else {\n");
            idl_fileOutPrintf (idl_fileCur(), "        %s%s[0]= '\\0';\n",
                    to_id, cIdName);
            idl_fileOutPrintf (idl_fileCur(), "    }\n");
        }
        break;
    ...
    void *userData)
{
    ...
    if (idl_typeSpecType(typeSpec) == idl_tbasic) {
        ...
        if(catsRequested || stacRequested) {
            ...
            if ((idl_typeSpecType(idl_typeDefRefered(idl_typeDef(typeSpec))) == idl_tarray) ||
                ...
                idl_fileOutPrintf (idl_fileCur(), "        __%s__copyOut(&from->%s, &to->%s);\n",
                        scopedName, idl_cId(name), idl_cId(name));
                idl_fileOutPrintf (idl_fileCur(), "    }\n");
                os_free(scopedName);
            } else {
                /* Calls itself for the actual type in case of typedef */
                idl_structureMemberOpenClose (scope, name, idl_typeDefActual(idl_typeDef(typeSpec)), userData);
            }
        }
        /* QAC EXPECT 3416; No side effect here */
    } else if (idl_typeSpecType(typeSpec) == idl_tenum) {
        c_char *cTypeName = idl_corbaCTypeFromTypeSpec (typeSpec);

        idl_fileOutPrintf (idl_fileCur(), "    to->%s = (%s)from->%s;\n",
                idl_cId(name), cTypeName, idl_cId(name));
        os_free(cTypeName);
        /* QAC EXPECT 3416; No side effect here */
        ...
        if(catsRequested) {
            ...
            snprintf(destin, sizeof(destin), "to->%s", name);
            idl_fileOutPrintf (idl_fileCur(), "    {\n");
            if (!idl_typeIsBoundedString(idl_typeArrayActual(idl_typeArray(typeSpec)))) {
                c_char *typeIdent = idl_scopedSplTypeIdent (idl_typeArrayActual(idl_typeArray(typeSpec)));

                idl_fileOutPrintf (idl_fileCur(), "        typedef %s _DestType", typeIdent);
                ...
                c_char source[256];
                snprintf(source, sizeof(source), "from->%s", name);
                idl_arrayElements (scope, name, idl_typeArray(typeSpec), source, destin, 1, stacRequested, catsRequested);
            }
            idl_fileOutPrintf (idl_fileCur(), "    }\n");
        }
        /* QAC EXPECT 3416; No side effect here */
        ...
        idl_fileOutPrintf (idl_fileCur(), "        const %s *src0 = (const %s *)from->%s;\n",
                scopedTypeIdent, scopedTypeIdent, idl_cId(name));
        idl_fileOutPrintf (idl_fileCur(), "        %s *dst0 = &to->%s;\n\n",
                sequenceIdent, idl_cId(name));
        ...
        if (idl_typeSeqMaxSize(idl_typeSeq(typeSpec)) == 0) {
            ...
            idl_fileOutPrintf (idl_fileCur(), "        DDS_sequence_replacebuf ((_DDS_sequence) dst0, (bufferAllocatorType)%s_allocbuf, %d);\n",
                    sequenceIdent, idl_typeSeqMaxSize(idl_typeSeq(typeSpec)));
        }
    ...
    os_boolean catsRequested)
{
    ...
    case idl_ttypedef:
        if(stacRequested) {
            ...
            if(idl_typeSpecType(idl_typeDefActual(idl_typeDef(typeSpec))) == idl_tarray) {
                 ...
                 */
                idl_arrayElements(
                    scope, name, idl_typeArray(idl_typeDefActual(idl_typeDef(typeSpec))),
                    from, to, indent, stacRequested, catsRequested);
            } else {
                idl_arrayLoopCopyBody(
                    typeArray, idl_typeDefActual(idl_typeDef(typeSpec)),
                    from, to, indent, scope, name, stacRequested, catsRequested);
            }
        } else {
            idl_printIndent (loopIndent + indent);
            ...
            case idl_tbasic:
                ...
                if (idl_typeBasicType(idl_typeBasic(idl_typeDefActual(idl_typeDef(typeSpec)))) == idl_string) {
                    c_ulong maxlen = idl_typeBasicMaxlen(idl_typeBasic(idl_typeDefActual(idl_typeDef(typeSpec))));
                    if (maxlen == 0) {
                        ...
                        idl_arrayLoopCopyIndex(typeArray);
                        idl_fileOutPrintf (idl_fileCur(), "[0], (const c_char *)%s", from);
                        idl_arrayLoopCopyIndex(typeArray);
                        idl_fileOutPrintf (idl_fileCur(), ", %d);\n", maxlen+1);
                        idl_printIndent (loopIndent + indent);
                        idl_fileOutPrintf (idl_fileCur(), "    %s", to);
                        idl_arrayLoopCopyIndex(typeArray);
                        idl_fileOutPrintf (idl_fileCur(), "[%d]= '\\0';\n", maxlen);
                        idl_printIndent (loopIndent + indent);
                        idl_fileOutPrintf (idl_fileCur(), "} else {\n");
                        idl_printIndent (loopIndent + indent);
                        idl_fileOutPrintf (idl_fileCur(), "    %s", to);
                        idl_arrayLoopCopyIndex(typeArray);
                        idl_fileOutPrintf (idl_fileCur(), "[0]= '\\0';\n");
                        idl_printIndent (loopIndent + indent);
                        idl_fileOutPrintf (idl_fileCur(), "}\n");
                    }
                } else {
    ...
    case idl_tbasic:
        ...
        /* QAC EXPECT 3416; No side effect here */
        if (idl_typeBasicType (idl_typeBasic(typeSpec)) == idl_string) {
            c_ulong maxlen = idl_typeBasicMaxlen(idl_typeBasic(idl_typeBasic(typeSpec)));
            if (maxlen == 0) {
                 idl_fileOutPrintf (idl_fileCur(), "DDS_string_replace ((*%s)", from);
                 idl_arrayLoopCopyIndex(typeArray);
                 idl_fileOutPrintf (idl_fileCur(), " ? (*%s)", from);
                 ...
                 idl_fileOutPrintf (idl_fileCur(), "    strncpy (&%s", to);
                 idl_arrayLoopCopyIndex(typeArray);
                 idl_fileOutPrintf (idl_fileCur(), "[0], %s", from);
                 idl_arrayLoopCopyIndex(typeArray);
                 idl_fileOutPrintf (idl_fileCur(), ", %d);\n", maxlen+1);
                 idl_printIndent (loopIndent + indent);
                 idl_fileOutPrintf (idl_fileCur(), "    %s", to);
    ...
    case idl_tseq:
        ...
        idl_fileOutPrintf (idl_fileCur(), "    %s *src0 = (%s *)%s;\n",
                scopedTypeIdent, scopedTypeIdent, source);
        idl_printIndent (total_indent);
        idl_fileOutPrintf (idl_fileCur(), "    %s *dst0 = &%s;\n\n",
                sequenceIdent, destin);
        idl_printIndent (total_indent);
    ...
    c_long indent)
{
    ...
    switch (idl_typeSpecType (typeSpec)) {
    case idl_tenum:
        corbaTypeName = idl_corbaCTypeFromTypeSpec (typeSpec);
        idl_printIndent (indent);
    ...
    case idl_tbasic:
        ...
        if (idl_typeBasicType (idl_typeBasic(typeSpec)) == idl_string) {
            ...
            if (maxlen > 0) {
                idl_fileOutPrintf(idl_fileCur(), "        strncpy(&(%s%d)._buffer[i%d][0], %s[i%d], %d);\n",
                        to, loop_index - 1, loop_index - 1, from, loop_index - 1, maxlen+1);
                idl_printIndent(indent);
                idl_fileOutPrintf(idl_fileCur(), "        (%s%d)._buffer[i%d][%u] = '\\0';\n",
                        to, loop_index - 1, loop_index - 1, maxlen);
            } else {
                idl_fileOutPrintf (idl_fileCur(), "        DDS_string_replace (%s[i%d] ? %s[i%d] : \"\", &(%s%d)._buffer[i%d]);\n",
                        from, loop_index-1, from, loop_index-1, to, loop_index-1, loop_index-1);
            }
        } else {
    ...
    case idl_tunion:
        ...
        idl_fileOutPrintf (idl_fileCur(), "        __%s__copyOut(&%s[i%d], &(%s%d)._buffer[i%d]);\n",
                scopedName, from, loop_index-1, to, loop_index-1, loop_index-1);
        os_free(scopedName);
        break;
    case idl_ttypedef:
        switch (idl_typeSpecType(idl_typeDefActual(idl_typeDef(typeSpec)))) {
    ...
    c_long indent)
{
    ...
    c_char *scopedName = idl_scopeStackC (scope, "_", name);
    c_char *scopedTypeName = idl_scopedSplTypeName(typeSpec);

    idl_fileOutPrintf (idl_fileCur(), "    DDS_unsigned_long size%d;\n", indent);
    idl_fileOutPrintf (idl_fileCur(), "    const %s *src%d = (const %s *)(*from);\n",
            scopedTypeName, indent, scopedTypeName);
    ...
    if (idl_typeSeqMaxSize(typeSeq) == 0) {
        ...
        idl_fileOutPrintf (idl_fileCur(), "    DDS_sequence_replacebuf ((_DDS_sequence)dst0, (bufferAllocatorType)%s_allocbuf, %d);\n",
                scopedName, idl_typeSeqMaxSize(typeSeq));
    }
    idl_fileOutPrintf (idl_fileCur(), "    dst0->_length = size0;\n");
    ...
    void *userData)
{
    ...
    case idl_tunion:
        ...
        idl_fileOutPrintf (idl_fileCur(), "__%s__copyOut(const void *_from, void *_to);\n",
                scopedName);
        scopedTypeName = idl_scopedTypeName(idl_typeDefActual(defSpec));
        idl_fileOutPrintf (idl_fileCur(), "void\n");
        idl_fileOutPrintf (idl_fileCur(), "__%s__copyOut(const void *_from, void *_to)\n",
                scopedName);
        idl_fileOutPrintf (idl_fileCur(), "{\n");
    ...
    void *userData)
{
    c_char *scopedName;
    c_char *scopedNameC;
    c_char *corbaTypeName = NULL;

    OS_UNUSED_ARG(userData);

    scopedName = idl_scopeStack(scope, "_", name);
    scopedNameC = idl_scopeStackC (scope, "_", name);
    ...
    if (idl_typeSpecType(idl_typeUnionSwitchKind(unionSpec)) == idl_tbasic) {
        ...
        corbaTypeName = idl_corbaCTypeFromTypeSpec (idl_typeUnionSwitchKind(unionSpec));
    ...
    if (idl_typeSpecType(typeSpec) == idl_tbasic) {
        ...
        if (idl_typeBasicType(idl_typeBasic(typeSpec)) == idl_string) {
            ...
            if (maxlen == 0) {
                ...
                idl_fileOutPrintf (idl_fileCur(), "        if (from->_u.%s) {\n", cIdName);
                idl_fileOutPrintf (idl_fileCur(), "            strncpy (&to->_u.%s[0], from->_u.%s, %d);\n", cIdName, cIdName, maxlen+1);
                idl_fileOutPrintf (idl_fileCur(), "            to->_u.%s[%d]= '\\0';\n", cIdName, maxlen);
                idl_fileOutPrintf (idl_fileCur(), "        } else {\n");
                idl_fileOutPrintf (idl_fileCur(), "            to->_u.%s[0]= '\\0';\n", cIdName);
                idl_fileOutPrintf (idl_fileCur(), "        }\n");
            }
        } else {
        ...
        if (idl_typeSpecType(idl_typeDefRefered(idl_typeDef(typeSpec))) == idl_tarray) {
            ...
            idl_fileOutPrintf (idl_fileCur(), "            __%s__copyOut(&from->_u.%s, &to->_u.%s);\n",
                    scopedTypeName, cIdName, cIdName);
            idl_fileOutPrintf (idl_fileCur(), "        }\n");
            idl_fileOutPrintf (idl_fileCur(), "        break;\n");
            os_free(scopedTypeName);
            /* QAC EXPECT 3416; No side effect here */
            ...
            /* Calls itself for the actual type in case of typedef */
            idl_unionCaseOpenClose (
                    scope, name, idl_typeDefRefered(idl_typeDef(typeSpec)), userData);
        }
        /* QAC EXPECT 3416; No side effect here */
    } else if (idl_typeSpecType(typeSpec) == idl_tenum) {
        scopedTypeName = idl_corbaCTypeFromTypeSpec (typeSpec);
        idl_fileOutPrintf (idl_fileCur(), "        to->_u.%s = (%s)from->_u.%s;\n",
                cIdName, scopedTypeName, cIdName);
        idl_fileOutPrintf (idl_fileCur(), "        break;\n");
        os_free(scopedTypeName);
        ...
        scopedTypeName = idl_scopedSplTypeIdent (idl_typeArrayActual(idl_typeArray(typeSpec)));
        ...
        scopedTypeName = idl_scopedSplTypeName(nextType);
        sequenceIdent = idl_c99SequenceIdent (idl_typeSeq(typeSpec));
        ...
        idl_fileOutPrintf (idl_fileCur(), "        const %s *src0 = (const %s *)from->_u.%s;\n",
                scopedTypeName, scopedTypeName, cIdName);
        idl_fileOutPrintf (idl_fileCur(), "        %s *dst0 = &to->_u.%s;\n\n",
                sequenceIdent, cIdName);
