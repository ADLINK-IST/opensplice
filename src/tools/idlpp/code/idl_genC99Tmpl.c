...
static c_char *idlpp_footer_template = NULL;
    ...
    void *userData)
{
    ...
    tmpl = (c_char *)userData;
    /* Only Header and Impl templates are supported */
    assert((strcmp(tmpl, "Header") == 0) || (strcmp(tmpl, "Source") == 0));

    /* Prepare file header template */
    snprintf(tmplFileName, sizeof(tmplFileName), "%s%cC99%c%sHeader", tmplPath, OS_FILESEPCHAR, OS_FILESEPCHAR, tmpl);
    ...
    snprintf(tmplFileName, sizeof(tmplFileName), "%s%cC99%c%sBody", tmplPath, OS_FILESEPCHAR, OS_FILESEPCHAR, tmpl);
    ...
    /* Prepare file footer template */
    snprintf(tmplFileName, sizeof(tmplFileName), "%s%cC99%c%sFooter", tmplPath, OS_FILESEPCHAR, OS_FILESEPCHAR, tmpl);
    /* QAC EXPECT 3416; No side effects here */
    if (os_stat(tmplFileName, &tmplStat) == os_resultSuccess) {
        if (os_access(tmplFileName, OS_ROK) != os_resultSuccess) {
            printf ("Template protection violation (%s)\n", tmplFileName);
            return (idl_abort);
        }

        /* QAC EXPECT 5007; will not use wrapper */
        idlpp_footer_template = os_malloc(tmplStat.stat_size+1);
        tmplFile = open(tmplFileName, O_RDONLY);
        nRead = (unsigned int)read(tmplFile, idlpp_footer_template, tmplStat.stat_size);
        memset(&idlpp_footer_template[nRead], 0, tmplStat.stat_size+1-nRead);
        close(tmplFile);
    }
    ...
    void *userData)
{
    ...
    if (idlpp_footer_template) {
        idl_tmplExp te;

        idlpp_inStream = idl_streamInNew(idlpp_footer_template, idlpp_macroAttrib);
        te = idl_tmplExpNew(idlpp_macroSet);
        idl_tmplExpProcessTmpl(te, idlpp_inStream, idl_fileCur());
        idl_streamInFree(idlpp_inStream);
        idl_tmplExpFree(te);

        os_free(idlpp_footer_template);
        idlpp_footer_template = NULL;
    }
    ...
    void *userData)
{
    idl_tmplExp te;
    idl_streamIn idlpp_inStream;
    idl_macro macro_uniqueTypeName;
    idl_macro macro_scopedTypeName;
    idl_macro macro_cleanFunctionName;
    char * scopeStack;
    char * scopedTypeName;
    char * cleanFunctionName;
    ...
    if (idl_keyResolve(idl_keyDefDefGet(), idl_typeUserScope(idl_typeUser(structSpec)), name) != NULL)
    {
        ...
        scopeStack = idl_scopeStack(scope, "_", name);
        macro_uniqueTypeName = idl_macroNew("uniquetypename", scopeStack);
        if (idl_typeSpecHasRef(idl_typeSpec(structSpec))) {
            size_t len = strlen(scopeStack) + strlen("__free") + 1;
            cleanFunctionName = os_malloc(len);
            sprintf(cleanFunctionName, "%s__free", scopeStack);
            macro_cleanFunctionName = idl_macroNew("cleanfunction", cleanFunctionName);
            os_free(cleanFunctionName);
        } else {
            macro_cleanFunctionName = idl_macroNew("cleanfunction", "NULL");
        }
        os_free(scopeStack);
        scopedTypeName = idl_scopeStack(scope, "::", name);;
        macro_scopedTypeName = idl_macroNew("scopedtypename", scopedTypeName);
        os_free(scopedTypeName);

        idl_macroSetAdd(idlpp_macroSet, macro_uniqueTypeName);
        idl_macroSetAdd(idlpp_macroSet, macro_scopedTypeName);
        idl_macroSetAdd(idlpp_macroSet, macro_cleanFunctionName);
        idlpp_inStream = idl_streamInNew(idlpp_template, idlpp_macroAttrib);
        idl_tmplExpProcessTmpl(te, idlpp_inStream, idl_fileCur());
        idl_streamInFree(idlpp_inStream);
        idl_tmplExpFree(te);
        idl_fileOutPrintf(idl_fileCur(), "\n");
        idl_macroSetRemove(idlpp_macroSet, macro_uniqueTypeName);
        idl_macroFree(macro_uniqueTypeName);
        idl_macroSetRemove(idlpp_macroSet, macro_scopedTypeName);
        idl_macroFree(macro_scopedTypeName);
        idl_macroSetRemove(idlpp_macroSet, macro_cleanFunctionName);
        idl_macroFree(macro_cleanFunctionName);
    }
...
/**
 * Actions to generate a c99_Foo_templates
 */
idl_program
idl_genC99TmplProgram (
    void *userData)
{
