    ...
    const char *to_id)
{
    ...
    case idl_octet:
        ...
        break;
    case idl_string:
        idl_fileOutPrintf (idl_fileCur(), "    DDS_string_replace (%s%s, &%s%s);\n",
	    from_id,
	    idl_cId(name),
            to_id,
	    idl_cId(name));
    ...
    c_long indent)
{
	...
	case idl_tbasic:
	    ...
	    if (idl_typeBasicType(idl_typeBasic(idl_typeDefActual(idl_typeDef(typeSpec)))) == idl_string) {
	        idl_fileOutPrintf (idl_fileCur(), "    DDS_string_replace (%s, &%s);\n", from, to);
	    } else {
    ...
    case idl_tseq:
        ...
        idl_fileOutPrintf (idl_fileCur(), "    %s *dst0 = &%s;\n\n",
            ...
            destin);
        idl_printIndent (total_indent);
        idl_fileOutPrintf (idl_fileCur(), "    size0 = c_arraySize((c_array)(%s));\n",
	    "src0");
	if (idl_typeSeqMaxSize(idl_typeSeq(typeSpec)) == 0) {
	    /* For unbounded sequence set maximum size */
            idl_printIndent (total_indent);
    ...
    c_long indent)
{
    ...
    if (size > 0) {
        idl_printIndent (indent);
        idl_fileOutPrintf (idl_fileCur(), "memcpy ((%s%d)._buffer,%s,size%d*%d);\n",
            to, loop_index-1, from, loop_index-1, size);
        return;
    }
    idl_printIndent (indent);
    idl_fileOutPrintf (idl_fileCur(), "{\n");
	...
	idl_printIndent (indent);
        /* QAC EXPECT 3416; No side effect here */
	if (idl_typeBasicType (idl_typeBasic(typeSpec)) == idl_string) {
            idl_fileOutPrintf (idl_fileCur(), "    DDS_string_replace (%s[i%d], &(%s%d)._buffer[i%d]);",
		from,
		loop_index-1,
		to,
		loop_index-1,
    ...
    void *userData)
{
    ...
    if (idl_typeSpecType(typeSpec) == idl_tbasic) {
        /* QAC EXPECT 3416; No side effect here */
        if (idl_typeBasicType(idl_typeBasic(typeSpec)) == idl_string) {
            idl_fileOutPrintf (idl_fileCur(), "        DDS_string_replace (from->_u.%s, &to->_u.%s);\n",
                idl_cId(name),
                idl_cId(name));
        } else {
