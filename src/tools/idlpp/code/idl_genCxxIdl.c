    ...
    void *userData)
{
    ...
    snprintf(tmplFileName, (size_t)sizeof(tmplFileName), "%s%c%s%ccorbaIdlInterfaceHeader", tmplPath, OS_FILESEPCHAR, orbPath, OS_FILESEPCHAR);
    ...
    snprintf(tmplFileName, (size_t)sizeof(tmplFileName), "%s%c%s%ccorbaIdlInterface", tmplPath, OS_FILESEPCHAR, orbPath, OS_FILESEPCHAR);
    /* QAC EXPECT 3416; No side effects here */
    if ((os_stat(tmplFileName, &tmplStat) != os_resultSuccess) ||
        (os_access(tmplFileName, OS_ROK) != os_resultSuccess)) {
        printf ("No template found or protection violation (%s)\n", tmplFileName);
...
idl_program
idl_genCxxIdlProgram(
    void)
{
    return &idl_genCxxIdl;
}
