...
void
idl_genISOCxx2SpliceTypeSetTestMode (
    c_bool val)
{
    test_mode = val;
}
    ...
    void *userData)
{
    ...
    if (idl_scopeStackSize(scope) == 0 || idl_scopeElementType (idl_scopeCur(scope)) == idl_tModule) {
        ...
        if(!test_mode)
        {
            ...
            idl_fileOutPrintf(
                idl_fileCur(),
                "extern %s v_copyin_result __%s__copyIn(c_type dbType, const class %s *from, struct _%s *to);\n",
                idl_dllGetMacro(),
                fullyScopedName,
                tmpfullyScopedName,
                fullyScopedName);
...
idl_structureMemberOpenClose(
    ...
    void *userData)
{
    c_char *cxxMemberName = idl_languageId(name);
    ...
    if (idl_typeSpecType(typeSpec) == idl_ttypedef||
        ...
        if(catsRequested) {
            ...
            typeDereffered = idl_typeDefResolveFully(typeSpec);
            idl_printIndent(indent_level);
            idl_fileOutPrintf(idl_fileCur(), "c_char %s", cxxMemberName);
            ...
            c_char *dbTypeName = idl_scopedSplTypeIdent(typeSpec);
        ...
        if(catsRequested) {
            ...
            if (idl_typeSpecType(idl_typeArrayActual (idl_typeArray(typeSpec))) != idl_tseq) {
                c_char *dbTypeName = idl_scopedSplTypeIdent(idl_typeArrayActual(idl_typeArray(typeSpec)));
                ...
                idl_fileOutPrintf(idl_fileCur(), "c_array %s", cxxMemberName);
            }
    ...
    /* generate code for a sequence mapping */
        idl_printIndent(indent_level);
        idl_fileOutPrintf(idl_fileCur(), "c_sequence %s;\n", cxxMemberName);
    } else {
    ...
    void *userData)
{
    ...
    if (idl_typeSpecType(idl_typeUnionSwitchKind(unionSpec)) == idl_tbasic) {
        os_free(fullyScopedName);
        fullyScopedName = idl_scopedTypeName(idl_typeUnionSwitchKind(unionSpec));
        idl_printIndent(indent_level);
        idl_fileOutPrintf (idl_fileCur(), "%s _d;\n", fullyScopedName);
    } else if (idl_typeSpecType(idl_typeUnionSwitchKind(unionSpec)) == idl_tenum) {
        os_free(fullyScopedName);
        fullyScopedName = idl_scopedSplTypeName(idl_typeUnionSwitchKind(unionSpec));
        idl_printIndent(indent_level);
        idl_fileOutPrintf(idl_fileCur(), "%s _d;\n", fullyScopedName);
    ...
    void *userData)
{
    ...
    /* generate code for a standard mapping or a typedef, enum, struct or union user-type mapping */
        ...
        if (idl_typeSpecType(idl_typeArrayActual (idl_typeArray(typeSpec))) != idl_tseq) {
            c_char *dbTypeName = idl_scopedSplTypeIdent(idl_typeArrayActual (idl_typeArray(typeSpec)));
            idl_fileOutPrintf(idl_fileCur(), "%s %s", dbTypeName, cxxMemberName);
            os_free(dbTypeName);
        } else {
            idl_fileOutPrintf(idl_fileCur(), "c_array %s", cxxMemberName);
        }
...
idl_enumerationOpen(
    ...
    void *userData)
{
    c_char *scopeStack = idl_scopeStack(scope, "_", name);
 ...
 */
idl_program
idl_genISOCxx2SpliceTypeProgram(
    void)
{
    return &idl_genISOCxx2SpliceType;
}
