    ...
    c_collectionType type)
{
    ...
    case C_SEQUENCE:
        ...
        if (type->maxSize == 0) {
	        ...
	        idl_scopeStack(idl_typeUserScope(idl_typeUser(typeSpec)), "::", idl_typeSpecName(subType)));
        } else {
            /* bounded sequence */
            snprintf(typeName,
                sizeof(typeName),
    ...
    idl_context context)
{
    ...
    if (presetScope == 0 &&
        (idl_fileMapObject(idl_fileMapDefGet(), context->fileName, c_baseObject(o)) == FALSE)) {
        /* Object not defined in processed file */
        return;
    }
    idl_metaObjectTrace(o, "idl_metaobject", context);
    switch (c_baseObject(o)->kind) {
...
idl_walk(
    ...
    idl_program program)
{
    idl_context context = os_malloc(C_SIZEOF(idl_context));
    idl_action action;
    ...
    if (action == idl_explore) {
        c_metaWalk(c_metaObject(base), idl_metaobject, context);
