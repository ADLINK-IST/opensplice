...
{
    OS_UNUSED_ARG(varg);
    return v_dbmsconnectNew(kernel, name, extStateName, qos, enable);
}
...
u_dbmsconnectNew (
    ...
    c_bool enable)
{
    return u_serviceNewSpecialized(wrapper, "DBMSConnectService", uri, id, timeout, name, qos, enable, NULL);
}
