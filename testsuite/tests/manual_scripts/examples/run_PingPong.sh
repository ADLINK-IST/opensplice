...
g_ppscript="$SRC_ROOT/ospli/testsuite/tests/new_pingpong/run_pingpong_test.xml"

function run_single_pingpong ()
{
    if [ $# -ne 9 ]; then
    ...
    echo -e "\n-------------------------------"
    echo "<<< PingPong in '$impl' between '$master' and '$slave' using '$runtype' and '$transport' >>>"

    STAF local STAX EXECUTE FILE "$g_ppscript" JOBNAME DDSPingPong FUNCTION pingpong_main ARGS "['$runtype', '$master', '$masterlog', '$masterospl', '$impl', '$slave', '$slavelog', '$slaveospl', '$impl', '$MY_STAF_PORT', '$MY_STAF_PORT']" WAIT RETURNRESULT DETAILS
    local result=$?
...
run_tests run_single_pingpong "$(plain_hostname)" "sp shm" "broadcast multicast ddsi2" "c cpp java corba_cpp corba_java "  2>&1 | tee "$PWD/screen.log"
