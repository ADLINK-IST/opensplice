...
run_tests run_single_dcps_tutorial "$(plain_hostname)" "sp shm" "broadcast multicast" "c cpp java corba_cpp corba_java" 2>&1 | tee "$PWD/screen.log"

function run_single_dlrl_tutorial ()
{
    if [ $# -ne 9 ]; then
        error_trace "ERROR: Wrong number of arguments to ${FUNCNAME[0]}"
        return 1
    ...
    echo -e "\n-------------------------------"
    echo "<<< Tutorial DLRL in '$impl' between '$master' and '$slave' using '$runtype' and '$transport' >>>"

    STAF local STAX EXECUTE FILE "$g_tscript" JOBNAME DDSTutorial FUNCTION tutorial_main ARGS  "['$runtype', '$master', '$masterospl', '$masterlog', '$impl', '$slave', '$slaveospl', '$slavelog', '$impl', 'dlrl', '$MY_STAF_PORT', '$MY_STAF_PORT']" WAIT RETURNRESULT DETAILS
    local result=$?
...
# Currently we assume that master host is always a localhost.
run_tests run_single_dlrl_tutorial "$(plain_hostname)" "sp shm" "broadcast multicast" "cpp java" 2>&1 | tee -a "$PWD/screen.log"
